...
PARSER_NEW_INPUT_ORDER = (
    ...
    # 3.1.0: the Whisper transcription of the original cover words.  Appended
    # last, like every earlier additive input, so stored link slots keep their
    # index.
    "cover_lyrics",
    # 3.1.0, Cover Studio: the locked lyrics block.  It must stay last here as
    # well, otherwise the canonical rebuild would move sockets that saved
    # workflows still address by index.
    "cover_lyrics_lock",
)
...
JSON_NEW_INPUT_ORDER = (
    ...
    # 3.1.0 audio-cover branch: which lyrics mode ran, how the score was
    # adapted for an instrumental cover, and the Whisper record the original
    # words came from.  Appended last for the same slot-stability reason.
    "cover_score_json",
    "cover_lyrics_json",
)
