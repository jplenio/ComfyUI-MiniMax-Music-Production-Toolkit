...
together with the cover nodes it controls: its `cover_enabled` output feeds the
...
Connect its `cover_enabled` BOOLEAN output to both:
...
dropdowns before execution even for lazy branches; see `LLM_PROVIDERS.md` for the
