...
NODE_INPUT_TOOLTIPS = {
    ...
    "MiniMaxSaveProductionJSON": {
        "metadata_json": "Complete reproducibility metadata produced by MiniMax Song Metadata. This becomes the core of the single canonical production JSON.",
...
NODE_DESCRIPTIONS = {
    ...
    "MiniMaxSongMetadata": "Builds the complete reproducibility metadata containing prompts, seeds, MiniMax settings, FlashSR/filter/repair/release settings and optional LLM system prompt. It is written once by Save Production JSON in the bundled workflow.",
    ...
    "MiniMaxSaveProductionJSON": "Writes one canonical, atomic production JSON after original audio, release FLAC, release MP3 and artwork have all been saved. The destination directory is configurable in MiniMax Output Paths (default json).",
