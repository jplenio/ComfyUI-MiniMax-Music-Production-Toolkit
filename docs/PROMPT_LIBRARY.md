...
The YuE2 family in `prompts/system/yue2/` now asks for a developed, numbered
arrangement in Style and the identical section-tag sequence in Lyrics. The
previous versions are archived in `prompts/YuE2-old/`. See [the YuE2 guide](YUE2.md)
for the format, instrumental example and how to refresh text in saved nodes.

In 3.0, **YuE2 Cover** reuses this family and appends cover-specific instructions.
SheetSage2 ABC and source identity accompany the brief, so the LLM can align
its arrangement with the transcribed music. The original ABC is passed unchanged
to YuE2; it is not generated by the LLM. The source filename owns the cover title.
...
Length now guides an approximate timed arrangement in all active system prompts.
YuE2's final Style carries the numeric target and matching Lyrics structure into
ABC planning and music generation. A one-minute request may finish beyond 60
seconds so its last phrase and decay can complete naturally; only the separate
